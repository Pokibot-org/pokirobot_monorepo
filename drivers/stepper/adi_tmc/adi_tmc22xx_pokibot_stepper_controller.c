...
static int tmc22xx_pokibot_stepper_set_reference_position(const struct device *dev, const int32_t value)
{
	return 0;
}
...
static int tmc22xx_pokibot_stepper_run(const struct device *dev, const enum stepper_direction direction)
{
	return 0;
}
...
static int tmc22xx_pokibot_stepper_set_micro_step_res(const struct device *dev,
					      enum stepper_micro_step_resolution micro_step_res)
{
	return 0;
}

static int tmc22xx_pokibot_stepper_get_micro_step_res(const struct device *dev,
					      enum stepper_micro_step_resolution *micro_step_res)
{
...
#define TMC22XX_POKIBOT_STEPPER_DEFINE(inst)                                                    \
	...
	DEVICE_DT_INST_DEFINE(inst, tmc22xx_pokibot_stepper_init, NULL, &tmc22xx_pokibot_data_##inst,              \
			      &tmc22xx_pokibot_config_##inst, POST_KERNEL, CONFIG_STEPPER_INIT_PRIORITY,   \
			      &tmc22xx_pokibot_stepper_api);

#define DT_DRV_COMPAT adi_tmc2209_pokibot
DT_INST_FOREACH_STATUS_OKAY_VARGS(TMC22XX_POKIBOT_STEPPER_DEFINE)
#undef DT_DRV_COMPAT
